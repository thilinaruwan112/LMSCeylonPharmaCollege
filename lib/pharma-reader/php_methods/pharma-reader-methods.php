...
{
    global $link;
    $questionDetails = GetPrescriptions()[$prescriptionId];
    ...
    if ($result->num_rows > 0) {
        ...
        if ($stmt_sql = mysqli_prepare($link, $sql)) {
            ...
            if (mysqli_stmt_execute($stmt_sql)) {
                ...
                $error = array('status' => 'error', 'message' => 'Something went wrong. Please try again later. ' . $link->error, 'answerStatus' => $answerStatus, 'answerGrade' => $answerGrade);
            }
...
function GetOverallGrade($loggedUser)
{
