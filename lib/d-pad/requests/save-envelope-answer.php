...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($fields as $field => $value) {
        $sql = "SELECT * FROM prescription_answer WHERE $field = '$value' AND `pres_id` LIKE '$prescriptionID' AND `cover_id` LIKE '$coverID'";
    ...
    // if ($UserLevel == "Student" && $UserLevel != "Student") {

    $sql = "SELECT `answer_id` FROM `prescription_answer_submission` WHERE `cover_id` = '$coverID' AND `pres_id` = '$prescriptionID' AND `answer_status` = 'Correct' AND `created_by` = '$LoggedUser'";
    ...
    if ($result->num_rows > 0) {
        ...
        $sql = "SELECT COUNT(answer_id) as count FROM prescription_answer_submission";
        $result = $link->query($sql);
        ...
        $sql = "INSERT INTO `prescription_answer_submission`(`answer_id`, `pres_id`, `cover_id`, `date`, `name`, `drug_name`, `drug_type`, `drug_qty`, `morning_qty`, `afternoon_qty`, `evening_qty`, `night_qty`, `meal_type`, `using_type`, `at_a_time`, `hour_qty`, `additional_description`, `created_by`, `answer_status`, `score`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
